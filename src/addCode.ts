...
export default function addCode(name:string, path:string, length:number, current:number, justName:string){
    ...
    fs.readFile(path, 'utf8', (err, data) => {
        ...
        if(data.split("$")[1]) {
            ...
            terminal.on('close', (code) => {
                setTimeout(async()=>{
                    await paragraph.addImage( `${ __dirname.split("/").pop() }/tmp/${justName}  - node  getScreenshot.js.png`)
            ...
            console.log(
                chalk.yellowBright.bold(
                    `${name}: aim set incorrectly or not set`
                )
            )
            return;
        }
